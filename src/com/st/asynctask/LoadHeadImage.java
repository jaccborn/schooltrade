...
public class LoadHeadImage {
	...
	public LoadHeadImage(Context context) {
	...
	public void ShowImageByAsyncTask(CircleImg mystoreimage, String url) {
		...
		mHandler = new Handler() {
			public void handleMessage(Message msg) {
				// 接收消息
					...
					System.out.println("得到imagename：" + imagename);
		...
		System.out.println("99999"+url+imagename);
		// 如果没有则 必须下载图片
		if (bitmap == null) {
			new ImageAsyncTask(mystoreimage).execute(url+imagename);
		} else {
			Drawable drawable = new BitmapDrawable(null, bitmap);
			mystoreimage.setImageDrawable(drawable);
		}
	...
	private class ImageAsyncTask extends AsyncTask<String, Void, Bitmap> {
		...
		public ImageAsyncTask(CircleImg headimage) {
